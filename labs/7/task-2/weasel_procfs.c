...
static struct proc_dir_entry *proc_weasel_whoami;
...
int proc_create_weasel(void) {
  ...
  proc_weasel_whoami = proc_create("whoami", 0, proc_parent, &ops);

  if (!proc_weasel_whoami) {
    pr_err("Error: Unable to create entry whoami!");
    return -ENOMEM;
  }

  proc_weasel_dcache = proc_create("dcache", 0, proc_parent, &dcache_ops);

  if (!proc_weasel_dcache) {
...
void proc_remove_weasel(void) {
  remove_proc_entry("dcache", proc_weasel_dcache);
  remove_proc_entry("whoami", proc_weasel_whoami);
